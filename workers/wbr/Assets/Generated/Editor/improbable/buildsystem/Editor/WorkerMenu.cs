...
namespace Improbable
{
    internal static class BuildWorkerMenu
    {
        ...
        [MenuItem(EditorConfig.ParentMenu + "/" + LocalMenu + "/UnityClient", false, EditorConfig.MenuOffset + 0)]
        public static void BuildLocalUnityClient()
        {
            MenuBuildLocal(new[] { "UnityClient" });
        }

        [MenuItem(EditorConfig.ParentMenu + "/" + CloudMenu + "/UnityClient", false, EditorConfig.MenuOffset + 0)]
        public static void BuildCloudUnityClient()
        {
            MenuBuildCloud(new[] { "UnityClient" });
        }

        [MenuItem(EditorConfig.ParentMenu + "/" + LocalMenu + "/UnityGameLogic", false, EditorConfig.MenuOffset + 1)]
        public static void BuildLocalUnityGameLogic()
        {
            MenuBuildLocal(new[] { "UnityGameLogic" });
        }

        [MenuItem(EditorConfig.ParentMenu + "/" + CloudMenu + "/UnityGameLogic", false, EditorConfig.MenuOffset + 1)]
        public static void BuildCloudUnityGameLogic()
        {
            MenuBuildCloud(new[] { "UnityGameLogic" });
        }

        [MenuItem(EditorConfig.ParentMenu + "/" + LocalMenu + "/All workers", false, EditorConfig.MenuOffset + 2)]
        public static void BuildLocalAll()
        {
            MenuBuildLocal(AllWorkers);
        }

        [MenuItem(EditorConfig.ParentMenu + "/" + CloudMenu + "/All workers", false, EditorConfig.MenuOffset + 2)]
        public static void BuildCloudAll()
        {
            MenuBuildCloud(AllWorkers);
        }

        [MenuItem(EditorConfig.ParentMenu + "/Clean all workers", false, EditorConfig.MenuOffset + 2)]
